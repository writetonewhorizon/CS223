...
// ----------------------------------------------------------------
// Global constant
const char* listsort_version = "optimized";
...
Cell sortFront(Cell x, int len, Cell* yp) {
	...
	} else if (len==2) {	// remove two cells and compare 
		Cell x2 = x->next;	// x2 is second cell 
		rest = x2->next;	// remainder of list 
		if (x->data <= x2->data) {
			x2->next = NULL; // break chain 
			*yp = x;		// x already linked to x2 
		} else {
			x2->next = x;	// link x2 to x 
			x->next = NULL;	// break chain 
			*yp = x2;		// x2 is head of new chain 
		}
	} else { 				// recursive case
